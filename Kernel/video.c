...
int getWidth() { return VBE_mode_info->width; }
  ...
  for (int j = 0; j < CHAR_HEIGHT; j++) {
    for (int i = 0; i < CHAR_WIDTH; i++) {
      ...
      if (bit == 1) {
        ...
        putPixel(color, x + 7 - i,
                 j + y); // For instance, put red pixel in row 0
      }
    }
  }
}
...
void moveUpScreen() {
  ...
  // Iterate from second line to last
  for (int y = VBE_mode_info->height - CHAR_HEIGHT; y < VBE_mode_info->height;
       y++) {
...
void putTab() {
  // Define the desired width for the tabulator (in this case, 4 spaces)
  int tabWidth = 4;

  // Calculate how many spaces are needed to reach the next multiple of
  // tabWidth
  int spacesToNextTab = tabWidth - (pointer_x % tabWidth);

  // Move the cursor forward by the calculated number of spaces
