...
#define BLOCK_PER_BYTE                                                         \
  (8 / 2) // 4 blocks per byte (00 00 00 00), 2 bits (FREE USED FIRST) for each
          // block. 00 free, 01 used, 10 first
...
void *malloc(const size_t size) {
  ...
  size_t blocksQty =
      (size + BLOCK_SIZE - 1) / BLOCK_SIZE; // Blocks quantity to assign
  ...
  for (int it_block = 0; it_block < BITMAP_SIZE; it_block++) {
    for (int it_bit = 0; it_bit < BLOCK_PER_BYTE; it_bit++) {
      ...
      int state =
          (bitmap[it_block] >> (it_bit * 2)) & 0x03; // Search for a free block
