...
uint64_t
    semPipeManager; // Semaphore ID for controlling access to the pipe array.
...
// Returns 0 if no pipe with the name exists, the position + 1 if it exists, and
...
static uint64_t indexValid(uint64_t pipeIndex) {
  return ((pipeIndex == 0 || pipeIndex > MAX_PIPES) &&
          pipes[pipeIndex - 1].available == FALSE)
             ? 0
             : 1;
}
